...
async function addSymptomstoPatient(req, res) {
    const { ssn, numDays, apptDate, apptTime, symptomType } = req.body;

    if (!ssn) return res.status(400).json({ success: false, error: 'Missing patientID' });
    ...
    try {
        const result = await recordSymptoms(ssn, numDays, apptDate, apptTime, symptomType);
